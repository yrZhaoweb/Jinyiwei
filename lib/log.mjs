 ...
 * Respects JINYIWEI_LOG=verbose|quiet for log level control.
 */
...
let logLevel = /** @type {"verbose" | "normal" | "quiet"} */ (
  process.env.JINYIWEI_LOG === "verbose" ? "verbose"
    : process.env.JINYIWEI_LOG === "quiet" ? "quiet"
    : "normal"
);
...
export function warn(message) {
  console.error(`  ${symbols.warn} ${yellow(message)}`);
}
...
/**
 * Print inline progress (overwrites current line on TTY).
...
export function progress(current, total, message) {
  if (logLevel === "quiet") return;
  const counter = dim(`[${current}/${total}]`);
  const line = `    ${symbols.bullet} ${counter} ${message}`;
  if (process.stdout.isTTY && current < total) {
...
/**
 * Print a verbose-only debug line.
 * @param {string} message
 */
...
export function banner(version) {
  if (logLevel === "quiet") return;
  console.log();
  console.log(`  ${bold(cyan("WatchAgent Jinyiwei"))} ${dim(`v${version}`)}`);
