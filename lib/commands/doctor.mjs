...
export function doctorCommand(args = []) {
  ...
  if (report.state.diagnostics.length > 0) {
    ...
    for (const item of report.state.diagnostics) {
      const severityLabel = item.severity === "error" ? t("diagnostic.severity.error") : t("diagnostic.severity.warning");
      const line = `${severityLabel}: ${t(item.message)}`;
