...
export function installCommand(args) {
  ...
  if (!isJson) log.step(7, totalSteps, t("install.installingSkills", { count: "?" }));
  if (skipSkills) {
    if (!isJson) log.info(log.dim("skipped (--skip-skills)"));
  } else {
    /** @type {{ skills: string[] }} */
    let manifest;
    try {
    ...
    if (!isJson) log.info(log.dim(`${manifest.skills.length} skills listed`));
  }
