...
export function buildStartGuideLines(input) {
  ...
  const lines = [
    ...
    "",
    t("startGuide.nextActions"),
    `1. ${t("startGuide.chatAgentForWork")}`,
    `2. ${t("startGuide.watchAgentForApprovals")}`,
    `3. ${t("startGuide.statusCommand")}`,
    `4. ${t("startGuide.configureCommand")}`,
    "",
    t("startGuide.controls"),
  ];
  ...
  if (agents.length > 0) {
    lines.push(`- ${t("startGuide.externalAgents", { agents: agents.join(", ") })}`);
  }

  if (groups.length > 0) {
    lines.push(`- ${t("startGuide.internalGroups", { groups: groups.join(" | ") })}`);
  }

  lines.push("");
  lines.push(defaultEntry === "main"
    ? t("startGuide.switchToChatAgent")
    : t("startGuide.alignedToWorkflow"));
...
export function buildStartGuidePayload(input) {
  return {
    ...
    workspace: input.workspace,
    nextSteps: [
      t("startGuide.openChatAgentForWork"),
      t("startGuide.watchAgentForApprovalsShort"),
      t("startGuide.statusHealthCheck"),
      t("startGuide.configureAdjust"),
    ],
