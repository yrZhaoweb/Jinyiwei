...
export async function initCommand() {
  ...
  try {
    ...
    const channelsRaw = (await ask(rl, t("init.prompt.externalChannels"))) || "feishu,telegram";
    const externalChannels = channelsRaw.split(",").map((/** @type {string} */ c) => c.trim()).filter(Boolean);

    const autoInstallRaw = (await ask(rl, t("init.prompt.autoInstallSkills"))) || "yes";
    const autoInstallListedSkills = autoInstallRaw.toLowerCase() === "yes";

    const listedSkillsManifest = (await ask(rl, t("init.prompt.skillsManifest"))) || "manifests/preinstalled-skills.json";
    ...
    log.info(t("init.backupCreated", { path: backupPath }));
    ...
    plugin.configSchema.properties.externalChannels.default = externalChannels;
    plugin.configSchema.properties.autoInstallListedSkills.default = autoInstallListedSkills;
    plugin.configSchema.properties.listedSkillsManifest.default = listedSkillsManifest;
