...
import { uniqueStrings, parseDelimitedList, formatChannelList } from "./utils.mjs";

/** @type {(command: string, args: string[], options?: object) => import("node:child_process").SpawnSyncReturns<string>} */
let spawnFn = spawnSync;

/**
 * Set a custom spawn function (for testing injection).
 * @param {(command: string, args: string[], options?: object) => import("node:child_process").SpawnSyncReturns<string>} fn
 */
export function setSpawnFn(fn) {
  spawnFn = fn;
}

/**
 * Get the current spawn function.
 * @returns {(command: string, args: string[], options?: object) => import("node:child_process").SpawnSyncReturns<string>}
 */
export function getSpawnFn() {
  return spawnFn;
}

// Re-export shared utilities for external consumers
export { uniqueStrings, parseDelimitedList, formatChannelList };
...
const MAX_PAYLOAD_LENGTH = 10_000_000; // 10MB max payload size to prevent memory exhaustion
...
function extractJsonPayload(text, maxDepth = 1000) {
  ...
  if (text.length > MAX_PAYLOAD_LENGTH) return null; // Prevent memory exhaustion
...
export function execOpenClaw(args, options = {}) {
  ...
  const result = spawnFn("openclaw", args, {
    stdio: "pipe",
    encoding: "utf8",
    timeout: 30_000,
  });
