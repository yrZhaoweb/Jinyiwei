...
function redactString(value) {
  ...
  return value
    .replace(/(sk-|ak-|token|password|secret|key|auth)[_-]?[a-zA-Z0-9]{8,}/gi, "[REDACTED]")
    .replace(/(["'])[\w-]{20,}(["'])/g, "\"$1[REDACTED]$2\"");
}

/**
 * Redact sensitive fields in audit params before logging.
 * @param {Record<string, any>} params
 * @returns {Record<string, any>}
 */
export function redactAuditParams(params) {
  const sensitiveFields = ["rationale", "output_or_rejection", "inputs", "constraints", "expected_outputs"];
