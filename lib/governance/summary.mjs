...
export function buildGovernanceSummary(input) {
  ...
  return {
    ...
    openclaw: {
      ...
      diagnostics: openclaw.diagnostics.map((d) => ({
        ...d,
        message: t(d.message),
        detail: d.detail ? t(d.detail) : undefined,
      })),
      guidance: openclaw.guidance.map((g) => t(g)),
      ready: blockingIssues.length === 0,
      blockingIssues: blockingIssues.map((item) => t(item.message)),
      warnings: warningIssues.map((item) => t(item.message)),
    },
  };
}
