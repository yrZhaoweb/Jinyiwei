...
export function hasOpenClaw() {
  const result = spawnSync("which", ["openclaw"], { stdio: "ignore" });
 ...
 * @param {string} channel - e.g. "feishu" or "telegram:123"
 * @param {{ dryRun?: boolean }} [options]
 */
export function agentBind(agentId, channel, options = {}) {
  return exec(["agents", "bind", "--agent", agentId, "--bind", channel], { dryRun: options.dryRun });
}
